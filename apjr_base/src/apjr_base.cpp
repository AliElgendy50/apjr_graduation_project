...
// #include <apjr_base/include/apjr_hw_interface.h>
#include <apjr_hw_interface.h>
...
void controlLoop(MyRobotHWInterface &hw, controller_manager::ControllerManager &cm, std::chrono::system_clock::time_point &last_time)
{
    ...
    hw.read(elapsed);       //reading encoder data
    cm.update(ros::Time::now(), elapsed);       //update joint states
    hw.write();             //publish left & right wheel vel msgs
}

int main(int argc, char** argv)
{
    ros::init(argc, argv, "apjr_base_node");
